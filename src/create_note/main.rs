...
const AGENT_NAME: &str = "mnemnk-obsidian-create-note";
...
struct ObsidianCreateNoteAgent {
    #[allow(unused)]
    config: AgentConfig,
}

impl ObsidianCreateNoteAgent {
    ...
    async fn process_input(&self, ctx: Value, data: Value) -> Result<()> {
        let Some(obj) = data.as_object() else {
            bail!("Invalid data for create_note command");
        };
    ...
    async fn create_note(&self, path: &str, content: &str) -> Result<()> {
        ...
        let response = client.put(&url).body(content.to_string()).send().await?;

        if response.status().is_success() {
            Ok(())
        } else {
            Err(anyhow::anyhow!(
                "Failed to create_note: {}",
                response.status()
            ))
        }
    }
...
async fn main() -> Result<()> {
    ...
    let mut agent = ObsidianCreateNoteAgent::new(config).await?;
    agent.run().await?;
