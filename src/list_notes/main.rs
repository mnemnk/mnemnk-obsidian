...
const AGENT_NAME: &str = "mnemnk-obsidian-list-notes";
...
struct ObsidianListNotesAgent {
    #[allow(unused)]
    config: AgentConfig,
}

impl ObsidianListNotesAgent {
    ...
    async fn process_input(&self, ctx: Value, data: Value) -> Result<()> {
        let Some(obj) = data.as_object() else {
            bail!("Invalid data for list_notes command");
        };
        let Some(value) = obj.get("value") else {
            bail!("Missing value");
        };
        let mut path = if value.is_string() {
            value.as_str().context("wrong string value")?.to_string()
        ...
        let value = self.list_notes(&path).await?;

        let files = value
            .as_object()
            .context("Invalid response from list_notes")?
            .get("files")
    ...
    async fn list_notes(&self, path: &str) -> Result<Value> {
...
async fn main() -> Result<()> {
    ...
    let mut agent = ObsidianListNotesAgent::new(config).await?;
    agent.run().await?;
